  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        title: '连续扫码',
        description:
          '使用 PDA 扫描物料二维码后，系统会按解析规则提取型号、批次、数量、生产日期、追溯码等信息。同型号会自动聚合显示，不同版本保留在展开明细中。',
      ...
      },
      {
        ...
        title: '入库记录',
        description:
          '入库记录按入库单分组，展开后只按型号聚合；同型号的不同版本不会重复计算型号数，具体版本会显示在每条明细中。',
      },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        title: '拆包同步',
        description:
          '订单管理内的拆包同步只同步本次拆包产生的标签数据；系统设置里的标签同步用于重新生成最近一个订单的拆包标签。',
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        title: '选择账套',
        description:
          '盘点前先选择无锡笃能或上海花栗鼠账套，ERP仓库会随账套自动确定，不需要手动选择。不同账套的未完成盘点会分别暂存，切换后不会混在一起。未开放的账套不能扫码或查询ERP。',
      },
      {
        id: '5.2',
        title: '扫码与数量修正',
        description:
          '每次扫码默认把标签数量计入实盘。包装数量与实际剩余数量不一致时，点开对应明细修改实盘数量；不需要再切换整包或拆包模式。',
      },
      {
        id: '5.3',
        title: '完成盘点',
        description:
          '完成盘点时，系统按唯一存货编码查询所选账套的ERP现存量，并用实盘数量减去ERP数量生成差异。核对失败时不会保存半张盘点单，扫码暂存会保留。',
      },
      {
        id: '5.4',
        title: '盘点记录',
        description:
          '盘点记录按盘点单分组，明细可查看批次、存货编码、生产日期和实盘数量。每张盘点单保存当时的账套及ERP库存快照，重新同步时直接生成包含“盘点明细”和“盘点差异”的同一个Excel文件，不会再次查询ERP。',
      },
      {
        id: '5.5',
        title: '盘点导出',
        description:
          '盘点页面完成盘点后会同步本次数据；系统设置里的“同步盘点单”只重新生成最近一张已保存盘点单，不会重新查询ERP，也不会把全部历史记录混入当前文件。',
      },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        title: '占位字段',
        description:
          '当二维码包含不需要使用的数据段时，可创建占位字段保持后续字段位置正确。占位字段不在物料明细中显示，也不会加入 Excel 导出。',
      },
    ],
  },
  {
    ...
    items: [
      {
        ...
        title: '电脑同步',
        description:
          '手机端填写电脑同步助手的 IP 和端口后，可同步入库单、出库单、盘点单和订单标签。同步成功后会显示生成的文件名。',
      },
      {
        id: '8.2',
        title: '实时同步与历史同步',
        description:
          '扫码页面保存后的同步只包含本次作业数据；系统设置里的单据同步可作为手动补发入口，其中盘点单和订单标签会重新生成最近一张记录。',
      },
      {
        id: '8.3',
        title: '配置备份',
        description:
          '配置备份包含解析规则、占位字段、仓库、出库单号规则、扫码提示音和同步服务器配置。物料绑定不参与配置备份或恢复，请在物料绑定页面使用独立的 Excel 导入与导出。',
      },
      {
        id: '8.4',
        title: '数据库备份',
