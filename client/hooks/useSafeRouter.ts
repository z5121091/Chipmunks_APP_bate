...
const MAX_SAFE_PAYLOAD_LENGTH = 2048;
...
const serializeParams = (params: Record<string, unknown>): string => {
  try {
    ...
    const encodedPayload = Base64.encode(jsonStr);

    if (encodedPayload.length > MAX_SAFE_PAYLOAD_LENGTH) {
      throw new Error(
        `参数过大（${encodedPayload.length} chars），请改用本地状态或持久化存储传递`
      );
    }
...
const deserializeParams = <T = Record<string, unknown>>(
  ...
  try {
    ...
    if (!isObjectPayload(parsed)) {
      logger.error(LOG_PREFIX, 'deserialize failed: payload is not an object');
      return null;
    }
    return parsed as T;
  } catch (error) {
...
export function useSafeSearchParams<T = Record<string, unknown>>(): T {
  ...
  const decodedPayloadParams = useMemo(() => {
    if (typeof payload !== 'string') {
      return null;
    }

    return deserializeParams<T>(payload);
  }, [payload]);
  ...
  if (decodedPayloadParams && Object.keys(decodedPayloadParams as Record<string, unknown>).length > 0) {
    return decodedPayloadParams;
  }
