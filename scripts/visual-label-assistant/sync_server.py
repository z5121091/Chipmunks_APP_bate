...
NATIVE_LABEL_PRINT_MODE = 'native_tsc_v1'
...
def build_native_text_bitmap(text_items):
...
def build_legacy_native_apm_label(record):
    ...
    scan_identifier = normalize_tspl_value(raw_version, 18) if raw_version else ''
    ...
    return setup_commands + build_native_text_bitmap(text_items) + print_commands


def build_native_apm_label(record, template=None):
    """使用当前可视化模板生成标签；模板无效时明确失败，避免静默打印错误版式。"""
    from print_engine import build_label_tspl
    from template_model import TemplateStore

    active_template = template if template is not None else TemplateStore().load_active()
    return build_label_tspl(active_template, record)
...
def submit_native_unpack_labels(records, print_job_id):
    ...
    with NATIVE_PRINT_LOCK:
        ...
        active_template = TemplateStore().load_active()
        copies_per_label = max(1, int(active_template.get('label', {}).get('copies', 1)))
        commands = b''.join(build_native_apm_label(record, active_template) for record in records)
        ...
        jobs[normalized_job_id] = {
            ...
            'copiesPerLabel': copies_per_label,
        }
        ...
        log(
            ...
            f'任务 {normalized_job_id}，{len(records)} 条，每条 {copies_per_label} 份，'
    ...
    def POST(self, name_suffix='', print_mode='', print_job_id='', **_ignored):
        ...
        try:
            ...
            with EXCEL_WRITE_LOCK:
                ...
                apply_excel_styles(sheet)
                if print_mode:
                    if print_mode != NATIVE_LABEL_PRINT_MODE:
...
class TrayIcon:
    ...
    def open_label_designer(self, _icon=None, _item=None):
        """以独立进程打开 Qt 编辑器，避免阻塞同步服务。"""
        try:
            if getattr(sys, 'frozen', False):
                command = [sys.executable, '--designer']
            else:
                command = [sys.executable, resource_path('app.py'), '--designer']
            subprocess.Popen(command, cwd=os.path.dirname(os.path.abspath(sys.executable)))
            log('已打开可视化标签编辑器')
        except Exception as error:
            log(f'打开可视化标签编辑器失败: {error}')
    ...
    def run(self):
        ...
        try:
            ...
            menu = pystray.Menu(
                ...
                pystray.MenuItem("可视化标签编辑器", self.open_label_designer),
